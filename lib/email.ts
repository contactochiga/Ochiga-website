...
import { formatPrivateReference, formatPartnershipReference } from "@/lib/leads/reference";
...
const acknowledgementCopy: Record<LeadType, { subject: string; body: string }> = {
  ...
  STRATEGIC_PARTNER: {
    subject: "Your Ochiga partnership enquiry has been received",
    // Unused by the HTML body (see partnershipAcknowledgementHtml below)
    // — kept accurate here since this record still supplies the email
    // subject line for this lead type.
    body: "Your partnership enquiry has been received and will be reviewed by the appropriate Ochiga team.",
  },
  GENERAL_CONTACT: {
...
function internalEmailHtml(payload: LeadPayload) {
  ...
  if (payload.lead_type === "STRATEGIC_PARTNER") return partnershipInternalHtml(payload);
  return `
...
function acknowledgementEmailHtml(payload: LeadPayload) {
  ...
  if (payload.lead_type === "STRATEGIC_PARTNER") return partnershipAcknowledgementHtml(payload);
...
// ---------------------------------------------------------------------
// Ochiga Partnerships — branded internal + acknowledgement emails, same
// quality bar as the Ochiga Private templates above. Applies to every
// STRATEGIC_PARTNER submission — the general Partnership intake now
// covers Capital / Buyer-Offtake / Delivery / Technology / Strategic
// categories, not only the original "strategic partner" profile page.
// ---------------------------------------------------------------------

function partnershipInternalHtml(payload: LeadPayload & { lead_type: "STRATEGIC_PARTNER" }) {
  const d = payload.strategic_partner;
  const rows: Array<[string, string]> = [
    ["Reference", payload.metadata.request_id],
    ["Name", payload.full_name],
    ["Email", payload.email],
    ...
    ["Partnership type", d.partner_type],
    ["Location", d.location || "—"],
    ["Description / proposal", d.capability_description],
    ["Website / LinkedIn", d.website_or_linkedin || "—"],
    ["Source page", payload.page_url || "—"],
    ["Submission date", payload.submitted_at],
  ];
  return `
    <div style="font-family:Arial,sans-serif;max-width:560px;">
      <p style="font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:#b3241b;">Ochiga Partnerships</p>
      <h2 style="font-size:18px;margin:8px 0 20px;">New Partnership Request — ${escapeHtml(d.partner_type)}</h2>
      <table>${rowsToHtml(rows)}</table>
    </div>
  `;
}

function partnershipAcknowledgementHtml(payload: LeadPayload & { lead_type: "STRATEGIC_PARTNER" }) {
  const d = payload.strategic_partner;
  const firstName = payload.full_name.trim().split(/\s+/)[0] || "there";
  const reference = formatPartnershipReference(payload.metadata.request_id, new Date(payload.submitted_at));
  const submittedDate = new Date(payload.submitted_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" });
  const partnershipsUrl = absoluteUrl("/partnerships");
  const privacyUrl = absoluteUrl("/privacy");
  ...
  const steps = [
    { number: "01", title: "Review", body: "We review what you bring or the opportunity you'd like to discuss." },
    { number: "02", title: "Structure", body: "Where there is a fit, we determine an appropriate commercial or operating structure." },
    { number: "03", title: "Align", body: "Scope, responsibilities and expectations are aligned before moving into execution." },
  ];
  ...
  <div style="background:#050505;padding:40px 16px;font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#141414;border-radius:6px;overflow:hidden;">
      <tr>
        <td style="padding:40px 40px 8px;">
          <p style="margin:0;font-size:11px;letter-spacing:0.16em;text-transform:uppercase;color:#b3241b;font-weight:600;">Ochiga Partnerships</p>
          <h1 style="margin:16px 0 0;font-family:Georgia,'Iowan Old Style','Times New Roman',serif;font-size:24px;line-height:1.25;color:#ffffff;">Partnership Request Received</h1>
        </td>
      </tr>
      <tr>
        <td style="padding:20px 40px 0;">
          <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#f6f3ec;">Thank you, ${escapeHtml(firstName)}.</p>
          <p style="margin:0 0 16px;font-size:14px;line-height:1.7;color:#d8d5cf;">Your enquiry has been received. Ochiga works through an ecosystem of landowners, capital relationships, buyers, delivery professionals, technology partners and strategic organisations across the Ochiga corporate ecosystem.</p>
          <p style="margin:0;font-size:14px;line-height:1.7;color:#d8d5cf;">Our team will review the information provided and contact you if anything further is required.</p>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td style="padding:12px 40px 40px;">
          <a href="${partnershipsUrl}" style="display:inline-block;background:#b3241b;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 28px;border-radius:4px;">Visit Partnerships</a>
        </td>
      </tr>
      <tr>
        <td style="padding:24px 40px 32px;border-top:1px solid rgba(255,255,255,0.1);">
          <p style="margin:0 0 4px;font-size:11px;letter-spacing:0.1em;text-transform:uppercase;color:#f6f3ec;">Ochiga Partnerships</p>
          <p style="margin:0 0 16px;font-size:12px;color:#8c887f;">Different capabilities. One ecosystem.</p>
          <p style="margin:0;font-size:11px;line-height:1.6;color:#57534a;">Ochiga · ${escapeHtml(companyInfo.location)} · <a href="${privacyUrl}" style="color:#57534a;text-decoration:underline;">Privacy Policy</a></p>
        </td>
      </tr>
    </table>
