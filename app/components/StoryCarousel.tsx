...
// Generic, reusable version of the Ochiga cinematic story-carousel
// interaction model (autoplay, capsule pause/play + dot→pill indicators,
// edge arrows, swipe, crossfade, reduced-motion). The homepage hero
// (app/components/HomeHeroCarousel.tsx) established this pattern; this
// component lets other pages (e.g. /development) use the exact same
// signature interaction without duplicating or forking that file.
import { useCallback, useEffect, useRef, useState, type TouchEvent } from "react";
import Link from "next/link";
import AbstractSurface from "@/app/components/AbstractSurface";

const DEFAULT_AUTOPLAY_MS = 8000;
const SWIPE_THRESHOLD_PX = 48;

const DEFAULT_SECTION_CLASSNAME =
  "relative flex min-h-screen flex-col justify-end overflow-hidden border-b border-ochiga-white/10 bg-ochiga-black px-6 pb-24 pt-40 md:px-10 md:pb-28";
...
export default function StoryCarousel({
  ...
  // autoplayMs, so pausing and resuming continues the same slide's
    ...
    >
          ...
          return (
            ...
            >
              ...
              {slide.image?.leftScrim ? (
                <div aria-hidden className="absolute inset-0" style={{ backgroundImage: slide.image.leftScrim }} />
              ) : null}
            </div>
          );
        })}
      ...
      <div className="relative z-10 mx-auto w-full max-w-cinematic">
        <div
          ...
          id={panelId}
          role="group"
          aria-label={multiSlide ? `Slide ${activeIndex + 1} of ${slides.length}: ${active.eyebrow}` : undefined}
      ...
      {multiSlide ? (
        <>
          {/* Story navigation arrows — pinned to the hero's left/right
              edges, vertically centered, independent of the control
              capsule below. */}
          ...
          {/* Floating control capsule — pause/play + one indicator per
              slide. Inactive slides are dots; the active slide expands
              into a short pill that fills left-to-right over the
              slide's autoplay duration. */}
          <div className="pointer-events-none absolute inset-x-0 bottom-6 z-10 flex justify-center px-6 md:bottom-8">
            ...
            >
              ...
              <div role="tablist" aria-label={`${ariaLabel} slides`} className="flex items-center gap-2 md:gap-2.5">
                {slides.map((slide, index) => {
                  const isActive = index === activeIndex;
                  return (
                    <button
